...
void OpenGLRenderer::Clear(float r, float g, float b, float a, float depth, int stencil)
{
	glClearDepthf(depth);
...
GLuint OpenGLRenderer::CreateTexture(const std::string& filename)
{
	...
	int32_t bpp = 0;
	uint8_t* imgdata = stbi_load(filename.c_str(), &textureWidth, &textureHeight, &bpp, STBI_rgb_alpha);
...
void OpenGLRenderer::PrintShaderError(GLuint shader)
{
	...
	if (infologLength)
	{
		...
		glGetShaderInfoLog(shader, infologLength, &charsWritten, strInfoLog);
...
void OpenGLRenderer::PrintProgramError(GLuint program)
{
	...
	if (infologLength)
	{
		const size_t bytes = infologLength;
		char* strInfoLog = new char[bytes + 1];
		memset(strInfoLog, 0, bytes + 1);
		glGetProgramInfoLog(program, infologLength, &charsWritten, strInfoLog);
		IApplication::Debug(strInfoLog);
		delete [] strInfoLog;
	}
}
